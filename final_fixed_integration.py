...
class RewardFocusedHeuristicPolicy(HeuristicPolicy):
    """Enhanced heuristic policy that focuses on reward-generating actions"""
    
    def __init__(self, seed=None, aggression=0.5):
        super().__init__(seed=seed, aggression=aggression)
        # Based on exploration: actions 5-9 can generate rewards
        self.reward_actions = [5, 6, 7, 8, 9]
        self.step_count = 0
    
    def select_action(self, state, info):
        self.step_count += 1
        
        # For first 10 steps, focus on reward-generating actions
        if self.step_count <= 10:
            return self.rng.choice(self.reward_actions)
        
        # Then use the parent heuristic logic but bias toward reward actions
        parent_action = super().select_action(state, info)
        
        # 30% chance to override with reward action
        if self.rng.random() < 0.3:
            return self.rng.choice(self.reward_actions)
...
class BalatroEnvironmentWrapper:
    ...
    def _initialize_environment(self):
        ...
        try:
            ...
            logger.info("✅ Real Balatro environment initialized (fixed version)")
    ...
    def reset(self, **kwargs):
        ...
        # Make state safe to avoid array comparison issues
...
def generate_real_episode(args_tuple):
    ...
    try:
        # Use fixed environment wrapper
        env_wrapper = BalatroEnvironmentWrapper()
        
        # Create policy with reward focus
        if policy_type == 'random':
            policy = RandomPolicy(action_space_size=10)
        elif policy_type == 'heuristic':
            policy = RewardFocusedHeuristicPolicy(aggression=0.5)  # Use enhanced version
        elif policy_type == 'enhanced':
            policy = EnhancedPolicy(aggression=0.6, economic_focus=0.4)
        elif policy_type == 'aggressive':
            policy = RewardFocusedHeuristicPolicy(aggression=0.9)  # Use enhanced version
        elif policy_type == 'conservative':
            policy = RewardFocusedHeuristicPolicy(aggression=0.2)  # Use enhanced version
        else:
            policy = RewardFocusedHeuristicPolicy(aggression=0.5)  # Default to enhanced
        ...
        metadata = {
            ...
            'env_type': 'fixed_balatro_gym',
            'non_zero_rewards': sum(1 for r in episode_rewards if r != 0)
        }
...
class RealBalatroDataGenerator:
    ...
    def generate_dataset(self, total_episodes=1000, policy_type='random', max_steps_per_episode=200):
        ...
        for i, (trajectory, episode_metadata) in enumerate(results):
            if trajectory is not None and episode_metadata is not None:
                ...
                with open(metadata_file, 'w') as f:
                    json.dump(episode_metadata, f, indent=2,cls=NumpyEncoder)
        ...
        with open(metadata_file, 'w') as f:
            json.dump(dataset_metadata, f, indent=2,cls=NumpyEncoder)
        ...
        with open(episodes_metadata_file, 'w') as f:
            json.dump(episode_metadatas, f, indent=2,cls=NumpyEncoder)
